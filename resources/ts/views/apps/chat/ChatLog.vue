...
const msgGroups = computed(() => {
  ...
  if (store.activeChat!.chat) {
    ...
    // console.log("active message change", store.activeChat!.chat.messages);
  ...
  <div class="chat-log pa-5">
    <div v-for="(msgGrp, index) in msgGroups" :key="msgGrp.senderId + String(index)"
      ...
      <div class="chat-body d-inline-flex flex-column"
        ...
        <p v-for="(msgData, msgIndex) in msgGrp.messages" :key="msgData.time" class="chat-content py-2 px-4 elevation-1"
          style="background-color: rgb(var(--v-theme-surface))" :class="[
            msgGrp.senderId === contact.id
              ? 'chat-left'
        ...
        <div :class="{ 'text-right': msgGrp.senderId !== contact.id }">
          <VIcon v-if="msgGrp.senderId !== contact.id" size="18" :color="
              resolveFeedbackIcon(
